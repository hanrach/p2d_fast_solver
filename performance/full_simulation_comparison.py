...
for j in range(0, repnum):
    for i in range(0, len(Mp)):
        ...
        U_naive, _, _, time_naive = p2d_fn_short(Np[i], Nn[i], Mp[i], Mn[i], Ms[i], Ma[i], Mz[i], delta_t, fn, jac_fn, Iapp, 3520)

        fn_fast, jac_fn_fast = p2d_init_fast(Np[i], Nn[i], Mp[i], Mn[i], Ms[i], Ma[i], Mz[i], delta_t, Iapp)
        U_fast, _, _, _, _, time_decoupled = p2d_fast_fn_short(Np[i], Nn[i], Mp[i], Mn[i], Ms[i], Ma[i], Mz[i], delta_t, fn_fast, jac_fn_fast, Iapp, 3520)
...
pickle.dump( [linsolve_slow_avg, linsolve_fast_avg, linsolve_reorder_avg,
              tot_slow_avg, tot_fast_avg, tot_reorder_avg,
